...
export const createYouTubeEmbedUrl = (videoId: string, options: YouTubeEmbedOptions = {}): string => {
  const {
    rel = 0,
    modestbranding = 1,
    fs = 1,
  ...
  const params = new URLSearchParams({
    ...
    ...Object.fromEntries(Object.entries(otherOptions).map(([k, v]) => [k, String(v)]))
  });
...
export const getYouTubeDimensions = (url: string): { width: string; height: string } => {
