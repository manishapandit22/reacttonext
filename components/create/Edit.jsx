...
import React, { memo, useRef, useState, useEffect, useContext, useCallback } from "react";
...
import { GameCreationSuccess } from "@/components/create_async/GameCreateSuccess";
...
import CharacterCreatorModal from "../create_async/CharacterStats";
import { defaultStatBlocks } from "../create_async/utils/DefaultBlocks";
import { classDescriptions } from "../create_async/utils/classDescription";
import Button from "../ui/Button";
import GameContextProvider from '../../contexts/GameContext';

const isProduction =
  process.env.NEXT_PUBLIC_VERCEL_ENV === "production" || process.env.NEXT_PUBLIC_VERCEL_URL?.includes('main')

const GameForm = memo(({ game_id }) => {
  ...
  async function fetchNPCs(game_id) {
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/ai-games/games/drafts/${game_id}/npcs/`
      );
      if (response.data && Array.isArray(response.data.success?.data)) {
  ...
  async function fetchLocations(game_id) {
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/ai-games/games/drafts/${game_id}/locations/`
      );
      if (response.data && Array.isArray(response.data.success?.data)) {
        return response.data.success.data;
      }
  ...
  useEffect(() => {
    async function loadGameDetails() {
      if (game && profile) {
        if (game?.user?.user_id !== profile?.user_id) {
          toast.error("You are not authorized to edit this game.");
          router.push(`/games`);
          return;
        }
      } else if (game && !profile) {
        toast.error("Please log in to edit games.");
        router.push(`/login`);
        return;
      }
      if (game && game?.user?.user_id && profile?.user_id && game?.user?.user_id == profile?.user_id) {
        const [locations, npcs] = await Promise.all([
          fetchLocations(game.game_id),
          fetchNPCs(game.game_id)
        ]);
        let game_data = {
          previewImage: null,
          previewImageType: "",
          ...
          initialInstructions: game.initial_instructions || "",
          gameTags: game.game_tags,
          savedStoryDocuments: game.story_documents,
          storyDocuments: [],
          locations: (locations && locations.length > 0 ? locations.map((loc) => ({
            name: loc.location_name || "",
            description: loc.location_description || "",
            images: [],
            savedImages: (loc.location_images || []).map(img => ({
              ...img,
              url: img.url === "undefined" ? undefined : img.url,
              type: img.type || (isYoutubeUrl(img.url) ? 'youtube' : 'image')
            })),
          })) : [
            {
              name: "",
          ...
          npcs: (npcs && npcs.length > 0 ? npcs.map((npc) => ({
            name: npc.npc_name || "",
        ...
        if (game.game_mechanics) {
          setGameMechanics(game.game_mechanics);
        }
        if (game.secrets_plot_twists) {
          setSecretsPlotTwists(game.secrets_plot_twists);
        }
        if (game.story_arcs) {
          setStoryArcs(game.story_arcs);
        }
        if (game.tone_writing_style) {
          setToneWritingStyle(game.tone_writing_style);
        }
        if (game.other_information) {
          setOtherInformation(game.other_information);
        }
      }
    }
    loadGameDetails();
  ...
  const handleLocationChange = (index, field, value) => {
    ...
    if (field === "images" && value.target.files) {
      newLocations[index].images = [
        ...
        ...Array.from(value.target.files).map(file => ({
          file,
          title: "",
          description: "",
          type: "image"
  ...
  const handleNPCChange = (index, field, value) => {
    ...
    if (field === "images" && value.target.files) {
      ...
      if (value) {
        newNPCs[index].class = newNPCs[index].class || "Default";
        newNPCs[index].characterSheet = {
          ...defaultStatBlocks[newNPCs[index].class],
          name: newNPCs[index].name,
          image_url: newNPCs[index].images[0]?.file || newNPCs[index].savedImages[0]?.url || "",
        ...
        setCurrentCharacter({
          ...newNPCs[index].characterSheet,
          playable: true,
          image: newNPCs[index].images[0]?.file || newNPCs[index].savedImages[0]?.url || "",
        });
        setIsModalOpen(true);
      } else {
      ...
      if (newNPCs[index].characterSheet) {
        ...
        newNPCs[index].characterSheet.image_url =
          newNPCs[index].images[0]?.file || newNPCs[index].savedImages[0]?.url || "";
      }
    }
  ...
  const removeNPCImage = (npcIndex, imageIndex) => {
    ...
    if (newNPCs[npcIndex].characterSheet) {
      newNPCs[npcIndex].characterSheet.image_url =
        newNPCs[npcIndex].images[0]?.file || newNPCs[npcIndex].savedImages[0]?.url || "";
    }
  ...
  const validateForm = () => {
    ...
    if (toneWritingStyle.trim().length == 0) {
      errors.toneWritingStyle = "This field is required";
    }

    if (secretsPlotTwists.trim().length == 0) {
    ...
    /**@author Saksham */
    /**@see enable it to enable charcter sheet */

    // const invalidCharacterSheets = gameData.npcs.some(
    //   (npc) => npc.playable && (!npc.class || !npc.characterSheet)
    // );
    // if (invalidCharacterSheets) {
    //   errors.npcs =
    //     "All playable characters must have a class and completed character sheet";
    // }

    return errors;
  };

  const handleCharacterModalSave = (abilities) => {
    if (selectedNPCIndex !== null) {
      const newNPCs = [...gameData.npcs];
      if (newNPCs[selectedNPCIndex].characterSheet) {
        newNPCs[selectedNPCIndex].characterSheet.abilities = {
    ...
    try {
      ...
      gameData.locations.forEach((location, index) => {
        ...
        location.images.forEach((image, i) => {
          ...
          if (image.type === 'youtube') {
            ...
            formData.append(
              `location[${index}][location_images][${i}][url]`,
              image.url
            );
          }
        });
      ...
      gameData.npcs.forEach((npc, index) => {
        ...
        npc.images.forEach((image, i) => {
          ...
          if (image.type === 'youtube') {
            formData.append(
              ...
              image.url // Use .url instead of .youtubeUrl
            );
          } else if (image.file instanceof File) {
            formData.append(
        ...
        if (npc.playable && npc.characterSheet) {
          // formData.append(
          //   `npc[${index}][character_sheet][name]`,
          //   JSON.stringify({
          ...
          // );
          formData.append(
            `npc[${index}][character_sheet]`,
            JSON.stringify(npc.characterSheet)
          );
        }
      });
      ...
      } else if (response.status === 207 && game_id) {
        // Handle partial success (207 status code)
        setProgress("Game Edited with warnings");
        
        // Extract and display warnings from the response
        if (response.data?.error?.message?.warnings) {
          const warnings = response.data.error.message.warnings;
          const warningMessage = `Game updated successfully, but with the following warnings:\n${warnings.join('\n')}`;
          setApiWarning(warningMessage);
        } else if (response.data?.error?.message?.message) {
          setApiWarning(response.data.error.message.message);
        }
        
        // Don't show success modal immediately for 207 - let user read warnings first
        setGameCreationId(game_id);
        // Show success modal after a delay to let user read the warnings
        setTimeout(() => {
          setIsSuccessVisible(true);
        }, 10000); // 5 seconds delay to read warnings
      } else {
  ...
  const generateImageId = () => Date.now().toString() + Math.random().toString(36).substring(2);
  ...
  const handleLocationImageChange = (imageData) => {
    ...
    if (imageData && imageData.moveToImages && imageData.image) {
      if (
        typeof selectedLocation === 'number' &&
        editingImageType === 'saved' &&
        typeof editingImageIndex === 'number'
      ) {
        newGameData.locations[selectedLocation].savedImages.splice(editingImageIndex, 1);
        newGameData.locations[selectedLocation].images.push(imageData.image);
        setSelectedLocationImage(newGameData.locations[selectedLocation].images.length - 1);
      ...
      if (isAdding) {
        newGameData.locations[selectedLocation].images = [
          ...newGameData.locations[selectedLocation].images,
          normalizedImageData
        ];
        setSelectedLocationImage(newGameData.locations[selectedLocation].images.length - 1);
      } else if (editingImageType && editingImageIndex !== null) {
        if (editingImageType === 'new') {
          newGameData.locations[selectedLocation].images[editingImageIndex] = normalizedImageData;
          setSelectedLocationImage(editingImageIndex);
        } else if (editingImageType === 'saved') {
          newGameData.locations[selectedLocation].savedImages[editingImageIndex] = normalizedImageData;
          setSelectedSavedImage(editingImageIndex);
        }
      }
    }
  ...
  const handleNPCImageChange = (imageData) => {
    ...
    if (imageData && imageData.moveToImages && imageData.image) {
      if (
        ...
        newGameData.npcs[selectedNPC].savedImages.splice(editingImageIndex, 1);
        newGameData.npcs[selectedNPC].images.push(imageData.image);
        setSelectedNPCImage(newGameData.npcs[selectedNPC].images.length - 1);
        setSelectedSavedImage(null);
      }
    } else {
      const normalizedImageData = {
      ...
      if (isAdding) {
        newGameData.npcs[selectedNPC].images.push(normalizedImageData);
      } else if (editingImageType && editingImageIndex !== null) {
        if (editingImageType === 'new') {
          newGameData.npcs[selectedNPC].images[editingImageIndex] = normalizedImageData;
        } else if (editingImageType === 'saved') {
          newGameData.npcs[selectedNPC].savedImages[editingImageIndex] = normalizedImageData;
        }
      }
    }
    setGameData(newGameData);
  ...
  useEffect(() => {
    if (
      ...
      typeof selectedLocation === "number" &&
      typeof selectedLocationImage === "number"
    ) {
      setIsOpen(true);
    }
  }, [selectedLocation, selectedLocationImage, selectedElement]);
  ...
  return (
    <>
      <GameCreationSuccess
        ...
        previewImage={gameData.previewImage || gameData.savedPreviewImage}
        game_id={gameCreationId}
        ...
        <div className="max-w-6xl mx-auto py-24 rounded-lg shadow">
          ...
          {apiError && (
            ...
            >
              ...
              <div className="ms-3 text-sm font-medium whitespace-pre-line">{apiError}</div>
              <button
                type="button"
                onClick={() => setApiError("")}
          ...
          {apiWarning && (
            <div
              id="alert-warning"
              className="flex items-center p-4 mb-4 text-[#92400e] rounded-lg bg-[#fef3c7] dark:bg-[#1f2937] dark:text-[#fbbf24]"
              role="alert"
            >
              <svg
                className="flex-shrink-0 w-4 h-4"
              ...
              <div className="ms-3 text-sm font-medium whitespace-pre-line">{apiWarning}</div>
              <button
                type="button"
                onClick={() => setApiWarning("")}
                className="ms-auto -mx-1.5 -my-1.5 bg-[#fef3c7] text-[#92400e] rounded-lg focus:ring-2 focus:ring-yellow-400 p-1.5 hover:bg-yellow-200 inline-flex items-center justify-center h-8 w-8 dark:bg-gray-800 dark:text-yellow-400 dark:hover:bg-gray-700"
          ...
          >
            <div className="flex gap-10 justify-between flex-col lg:flex-row">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className="relative group flex items-center">
                            <button className="px-2 py-2 text-white rounded ">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24"
                                fill="currentColor"
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div>
                  <label className={labelStyling}> Gameplay Initial Instructions</label>
                  <textarea
                    value={gameData.initialInstructions}
                    onChange={(e) =>
                      setGameData({ ...gameData, initialInstructions: e.target.value })
                    }
            ...
            <div className="mt-10 p-6 rounded-lg shadow-md">
              <h3 className="text-xl font-bold text-jacarta-100 mb-4">Game Details</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className={labelStyling}>Game Mechanics (Optional)</label>
                  <textarea
                    value={gameMechanics}
                ...
                <div>
                  <label className={labelStyling}>Secrets, Plot Twists, and When/How to Reveal Them</label>
                  <textarea
                    value={secretsPlotTwists}
                    onChange={(e) => setSecretsPlotTwists(e.target.value)}
                    rows={5}
                    style={{ resize: "vertical", minHeight: "9.5rem" }}
                    className="custom-input mt-2 mb-2"
                  />
                ...
                <div>
                  <label className={labelStyling}>Story/Quests Arcs/Chapters (Optional)</label>
                  <textarea
                    value={storyArcs}
                    onChange={(e) => setStoryArcs(e.target.value)}
                    rows={5}
                    style={{ resize: "vertical", minHeight: "9.5rem" }}
                    className="custom-input mt-2 mb-2"
                  />
                ...
                <div>
                  <label className={labelStyling}>Tone and Writing Style (Provide Examples)</label>
                  <textarea
                    value={toneWritingStyle}
                    onChange={(e) => setToneWritingStyle(e.target.value)}
                    rows={5}
                    style={{ resize: "vertical", minHeight: "9.5rem" }}
                    className="custom-input mt-2 mb-2"
                  />
                ...
                <div>
                  <label className={labelStyling}>Other Information (Optional)</label>
                  <textarea
                ...
                <div className="grid grid-cols-3 gap-4 mt-3">
                  {gameGenres.map((tag) => (
                    <div
                      ...
                      className={`flex items-center justify-center border-2 cursor-pointer gap-2 px-3 py-2 rounded-3xl font-semibold hover:text-accent hover:border-accent-dark${gameData.gameTags.includes(tag)
              ...
              {gameData.locations.map((location, index) => (
                ...
                >
                  ...
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) => handleLocationChange(index, "images", e)}
                    className="w-[1rem] hidden"
                  />
                  <MultipleImageCardPreview
                    ...
                    setIsOpen={setIsOpen}
                    setEditingImageType={setEditingImageType}
            ...
            <div>
              <div className="relative group flex items-center max-w-[150px]">
                ...
                <button className="ml-2 text-white  rounded ">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                ...
                <div
                  className="z-10 absolute top-full left-1/2 
                        ...
                        group-hover:visible"
                >
                  For 'Locations' And 'NPCs' Helps Generate dynamic
                  descriptions and visuals using AI to bring your game's
                  story to life, enhancing immersion with memory-driven
                  narratives. ...Because a journey through your world feels
                  more epic when the scenery matches the story. 🌄
                </div>
              </div>
              ...
              {gameData.npcs.map((npc, index) => (
                ...
                >
                    ...
                    <div className="py-4 flex items-center gap-4">
                      ...
                      <div className="w-full hover:ring-accent/10 focus:ring-accent dark:border-jacarta-600 dark:bg-jacarta-700 dark:text-white dark:placeholder:text-jacarta-300  relative">
                        <select
                          ...
                          value={npc.class}
                          onChange={(e) =>
                            handleNPCChange(index, "class", e.target.value)
                          }
                        ...
                        >
                          ...
                          {Object.keys(classDescriptions).map(
                            (className) => (
                        ...
                        {hoveredClass && (
                          <div className="absolute z-10 mt-1 bg-white dark:bg-jacarta-800 border border-gray-300 dark:border-gray-700 rounded-md shadow-lg p-4 max-w-xs">
                            ...
                            <p className="text-sm">
                              {classDescriptions[hoveredClass]}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  <input
                    ...
                    id={`npc-image-${index}`}
                    accept="image/*"
                    onChange={(e) => handleNPCChange(index, "images", e)}
                    className="w-[1rem] hidden"
                  />
                  <MultipleImageCardPreview
                    files={npc.images}
                    ...
                    setEditingImageIndex={setEditingImageIndex}
                    onRemove={(imageIndex) =>
                      removeNPCImage(index, imageIndex)
                    }
                    onRemoveURL={(imageIndex) =>
                      removeNPCImageURL(index, imageIndex)
                    }
                    setSelectedNPCImageId={setSelectedLocationImage}
                  />
            ...
            <Button
              ...
              className={`mx-auto border-accent gap-3 glow text-white bg-transparent border-2 font-semibold tracking-wide rounded-md hover:bg-green-600 ${triggerJelloAnimations && "jello-horizontal"
                }`}
            >
              Edit Game{" "}
      ...
      {/* <ImageModal
        selectedData={(() => {
          if (selectedElement === "NPC") {
            if (editingImageType === 'saved' && typeof selectedNPC === 'number' && typeof editingImageIndex === 'number') {
              return gameData.npcs?.[selectedNPC]?.savedImages?.[editingImageIndex] ?? null;
            } else if (editingImageType === 'new' && typeof selectedNPC === 'number' && typeof editingImageIndex === 'number') {
              return gameData.npcs?.[selectedNPC]?.images?.[editingImageIndex] ?? null;
            } else if (savedImage) {
              return {
                ...gameData.npcs?.[selectedNPC]?.savedImages?.[selectedSavedImage],
                type: isYoutubeUrl(gameData.npcs?.[selectedNPC]?.savedImages?.[selectedSavedImage]?.url) ? 'youtube' : 'image',
                youtubeUrl: isYoutubeUrl(gameData.npcs?.[selectedNPC]?.savedImages?.[selectedSavedImage]?.url)
                  ? gameData.npcs?.[selectedNPC]?.savedImages?.[selectedSavedImage]?.url
                  : ''
              };
            } else {
              return gameData.npcs?.[selectedNPC]?.images?.[selectedNPCImage] ?? null;
            }
          } else {
            if (editingImageType === 'saved' && typeof selectedLocation === 'number' && typeof editingImageIndex === 'number') {
              return gameData.locations?.[selectedLocation]?.savedImages?.[editingImageIndex] ?? null;
            } else if (editingImageType === 'new' && typeof selectedLocation === 'number' && typeof editingImageIndex === 'number') {
              return gameData.locations?.[selectedLocation]?.images?.[editingImageIndex] ?? null;
            } else if (savedImage) {
              return {
                ...gameData.locations?.[selectedLocation]?.savedImages?.[selectedSavedImage],
                type: isYoutubeUrl(gameData.locations?.[selectedLocation]?.savedImages?.[selectedSavedImage]?.url) ? 'youtube' : 'image',
                youtubeUrl: isYoutubeUrl(gameData.locations?.[selectedLocation]?.savedImages?.[selectedSavedImage]?.url)
                  ? gameData.locations?.[selectedLocation]?.savedImages?.[selectedSavedImage]?.url
                  : ''
              };
            } else {
              return gameData.locations?.[selectedLocation]?.images?.[selectedLocationImage] ?? null;
            }
          }
        })()}
