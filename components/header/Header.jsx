...
export default function Header() {
  ...
  const { gamePointCoupons, profile } = useUser();
  ...
  const hasNoGamePoints =
    !profile?.subscription &&
    (profile?.game_points === 0 || profile?.game_points === undefined);

  return (
    <>
      <header
        className={`js-page-header fixed top-0 z-[1000] w-full backdrop-blur transition-colors  ${
        ...
        <div className="flex items-center px-6 py-6 xl:px-12 2xl:px-24">
          ...
          <Link href="/" className="shrink-0 flex items-center gap-2">
            <Image
              ...
              alt="OpenBook.Games | Game Marketplace"
            />
            <Image
              width={40}
              height={40}
              ...
              alt="OpenBook.Games | Game Marketplace"
            />
            <p className="text-[#FA8704] text-2xl font-bold">OpenBook</p>
          </Link>
          ...
          <div className="ml-auto hidden sm:block h-[38px] animate-gradient--no-text-fill rounded-md  text-center shadow-md  hover:shadow-lg animate-gradient--no-text-fill animate-gradient overflow-hidden !bg-clip-border  transition-all">
            <Link
              href={user ? `/create` : "#"}
          ...
          {/* Menu / Actions */}
          <div className="js-mobile-menu invisible lg:visible fixed inset-0 z-[1000] items-center bg-white opacity-0 dark:bg-jacarta-800 lg:relative lg:inset-auto lg:flex lg:bg-transparent lg:opacity-100 dark:lg:bg-transparent ">
            {/* Mobile Logo / Menu Close */}

            <div className="t-0 fixed left-0 z-[1000] flex w-full items-center justify-between bg-white p-6 dark:bg-jacarta-800 lg:hidden">
              {/* Mobile Logo */}
            ...
            <div className="ml-8 hidden lg:flex xl:ml-12">
              <Link
                href="/wallet"
                className={`js-nav-dropdown group-dropdown relative mx-2 flex h-10 w-10 items-center justify-center rounded-full border border-jacarta-100 bg-white transition-colors hover:border-transparent focus:border-transparent dark:border-transparent dark:bg-white/[.15] ${
                  hasNoGamePoints
                    ? "animate-bounce border-red-500 ring-2 ring-red-400"
                    : "hover:bg-accent dark:hover:bg-accent"
                }`}
                style={hasNoGamePoints ? { animationDuration: "1.2s" } : {}}
                title={hasNoGamePoints ? "No game points! Buy now to keep playing." : undefined}
              >
                ...
                <svg
                  ...
                  className={`h-5 w-5 transition-colors ${
                    hasNoGamePoints
                      ...
                      : "fill-jacarta-700 group-hover:fill-white dark:fill-white"
                  }`}
                  style={hasNoGamePoints ? { animationDuration: "1.2s" } : {}}
                >
            ...
            {user ? (
              ...
              >
                <div className="relative w-10 h-10 rounded-full overflow-hidden object-cover">
                  <Image src={user.profile_photo} fill alt="" />
                </div>
              </Link>
            ) : (
              <Link
